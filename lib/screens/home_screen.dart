...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _loadApplicationsSummary() async {
    try {
      ...
      if (mounted) {
        setState(
          () => _applicationsInReview = summary['enRevision'] as int? ?? 0,
        );
      }
    } catch (_) {}
  }
          ...
          child: Row(
            children: [
              ...jobsProvider.categories.take(4).map((category) {
                final isSelected =
                    jobsProvider.selectedCategoryId == category.id;
                return Padding(
  ...
  Widget _buildJobsList() {
    return Consumer<JobsProvider>(
      builder: (context, jobsProvider, _) {
        ...
        return Column(children: jobs.map((job) => _JobCard(job: job)).toList());
      },
    );
  }
}
...
class _JobCardState extends State<_JobCard> {
  ...
  Future<void> _toggleSave() async {
    ...
    try {
      ...
      final jobsProvider = context.read<JobsProvider>();
      await jobsProvider.loadRecommendedJobs(
        categoryId: jobsProvider.selectedCategoryId,
      );
